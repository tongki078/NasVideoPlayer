    ...
    try:
        ...
        if best_match:
            ...
            info = {
                ...
                "title": d_resp.get('title') or d_resp.get('name'),  # <-- 이 줄을 반드시 추가!
        ...
        for i in range(0, total, batch_size):
            ...
            for task, info in results:
                ...
                if info.get('failed'):
                    ...
                    batch_success += 1
                    # info.get('tmdbId') 등이 있는 리스트에 tmdbTitle을 추가합니다.
                    up = (
                        ...
                        info.get('tmdbId'),
                        info.get('title') or info.get('name')  # <-- 이 부분이 한글 제목입니다.
                    )

                    # SQL의 UPDATE 문에도 tmdbTitle=? 을 추가해 주세요.
                    cursor.executemany(
