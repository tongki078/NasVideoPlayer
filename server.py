...
# [개선] 에피소드 번호 추출 패턴 대폭 강화
REGEX_EP_MARKER_STRICT = re.compile(r'(?i)(?:(?<=[\uac00-\ud7af\u3040-\u30ff\u4e00-\u9fff])|[.\s_-]|^)(?:第?\s*S(\d+)E(\d+)(?:[-~]E?\d+)?|第?\s*S(\d+)|第?\s*E(\d+)(?:[-~]\d+)?|(\d+)\s*(?:화|회|기|부|話)|Season\s*(\d+)|Episode\s*(\d+)|시즌\s*(\d+))(?:\b|[.\s_-]|$)')
...
REGEX_JUNK_KEYWORDS = re.compile(r'(?i)\s*(?:더빙|자막|극장판|BD|TV|Web|OAD|OVA|ONA|Full|무삭제|감독판|확장판|익스텐디드|등급고지|예고편|(?<!\S)[상하](?!\S)|극장판\s*\d+기|특집\s*다큐|\d+부작)\s*')
                ...
                if info.get('failed'):
                    ...
                    if 'seasons_data' in info:
                        ...
                        if ep_batch:
                            ...
                            log("METADATA", f"📺 '{task['sample_name']}' 에피소드 {len(ep_batch)}개 정보 및 Still 이미지 적용 완료")
            conn.commit()
            conn.close()
    ...
    <body>
        ...
        <script>
            async function loadFailures() {
                ...
                for (const [orig, info] of Object.entries(data)) {
...
def get_diagnostics():
    return jsonify(MATCH_DIAGNOSTICS)
...
def _rebuild_fast_memory_cache():
    ...
    for cat in ["movies", "foreigntv", "koreantv", "animations_all", "air"]:
        ...
        for row in all_rows:
            ...
            # [추가] 포스터 정보가 없는 항목은 과감히 제외 (사용자 거부감 방지, 방송중 제외)
            if not poster and cat != 'air':
                continue
